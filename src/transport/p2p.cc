...
ncclResult_t ncclP2pAllocateShareableBuffer(size_t size, ncclIpcDesc *ipcDesc, void **ptr) {
  if (ncclCuMemEnable()) {
    ...
    NcclTmsImpl::instance().registerAlloc(*ptr, size, ipcDesc->cuDesc.data, NcclTmsIpcMode::EXPORTER);
#else
...
ncclResult_t ncclP2pImportShareableBuffer(struct ncclComm *comm, int tpPeer, size_t size, ncclIpcDesc *ipcDesc, void **devMemPtr) {
  if (ncclCuMemEnable()) {
    ...
    *devMemPtr = (void *)dptr;
    ...
    NcclTms::instance().registerAlloc(dptr, size, cuDesc->data, NcclTmsIpcMode::IMPORTER);
#else
    return ncclInternalError;
#endif
  } else {
